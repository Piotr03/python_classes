...
class Account:
    ...
    def deposit(self, amount):
        ...
        if amount < 0:
            raise ValueError("Impossible to deposit a negative amount, " + str(amount))
        self._balance += amount

    def charge(self, amount):
        #TODO add throwing exception when amount is invalid or there are insufficient funds
        if amount < 0:
            raise ValueError("Impossible to charge a negative amount, " + str(amount))
        if amount > self._balance:
            raise ValueError("Insufficient funds, impossible to charge more than available balance, " + str(amount))
        self._balance -= amount
...
a_from = b.find_account(1002)
print('account from')
print(a_from)

b.transfer(1002, 1003, 20)
